...
import { AgentType, AgentStatus, AgentConfig, Agent } from '../../types/agent.types';
import { TaskStatus, TaskPriority, Task } from '../../types/task.types';
import { MessageType, AgentMessage, CollaborationSession } from '../../types/message.types';
 ...
 */
export class AgentConfigValidator extends BaseValidator<AgentConfig> {
...
export class AgentValidator extends BaseValidator<Agent> {
  private configValidator = new AgentConfigValidator();
 ...
 * Task validator
 */
export class TaskValidator extends BaseValidator<Task> {
 ...
 */
export class AgentMessageValidator extends BaseValidator<AgentMessage> {
 ...
 */
export class CollaborationSessionValidator extends BaseValidator<CollaborationSession> {
...
export class ValidationUtils {
  ...
  static validateOrThrow<T>(validator: BaseValidator<T>, data: T, context?: string): void {
    ...
    if (!result.isValid) {
      ...
      throw new ValidationError(
        `Validation failed${contextMsg}: ${result.errors.join(', ')}`,
  ...
  static validateMany<T>(validator: BaseValidator<T>, items: T[]): ValidationResult {
    ...
    const allErrors = results.flatMap((result, index) => 
      result.errors.map(error => `Item ${index}: ${error}`)
    );
